...
import { SuppliersFiltersDialogComponent } from '../suppliers-filters-dialog/suppliers-filters-dialog.component';


@Component({
  selector: 'app-suppliers-grid-main',
  standalone: true,
  imports: [HeaderComponent, SearchInputTextComponent, NgIcon, ChatBubbleComponent, NgClass,
     DialogModule,
     FormsModule],
  templateUrl: './suppliers-grid-main.component.html',
  styleUrl: './suppliers-grid-main.component.scss',
...
export default class SuppliersGridMainComponent extends BaseForm implements OnInit {
  ...
  async openDialog() {
    ...
    const dialogRef = this.dialog.open(SuppliersFiltersDialogComponent, {
      ...
      .then((result: {name: string, id: number, email: string, phone: string, active: boolean}) => {
        if(result){
          if(result.name || result.id || result.email || result.phone || result.active){
            this.filter(result.name, result.id, result.email, result.phone, result.active);
          }
        }
      })
      .catch((error: any) => {
  ...
  edit(id: number){
    this.router.navigate([`/dashboard/suppliers/detail/edit/${id}`]);
  }
  ...
  filter(name?: string, id?: number, email?: string, phone?: string, active?: boolean){
    let filter = '';
    if(id){
    ...
    if(phone){
      filter = filter.concat(`&phone=${phone}`);
    }
    if(active){
