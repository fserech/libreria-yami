...
import { URL_BRANDS, URL_CATEGORIES } from '../../../../../shared/constants/endpoints';
...
@Component({
  selector: 'app-brands-grid-main',
  standalone: true,
  imports: [HeaderComponent, SearchInputTextComponent, NgIcon, ChatBubbleComponent, NgClass,
     DialogModule,
     FormsModule],
  templateUrl: './brands-grid-main.component.html',
  styleUrl: './brands-grid-main.component.scss',
...
export default class BrandsGridMainComponent extends BaseForm implements OnInit {
 ...
 form: FormGroup;
   ...
   async openDialog() {
     ...
     const dialogRef = this.dialog.open(BrandListComponent, {
   ...
   filter(name?: string, id?: number, initPrice?: number, endPrice?: number, active?: boolean){
     ...
     if(name){
       filter = filter.concat(`&brandName=${name}`);
     }
