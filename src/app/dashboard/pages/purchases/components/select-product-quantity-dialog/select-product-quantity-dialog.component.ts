...
export class SelectProductQuantityDialogComponent {
  ...
  load: boolean = false;
  product: Product;
  isPurchase: boolean = false; // ✅ Para identificar si es compra o venta
  productsList: {product: Product, quantity: number }[];

  constructor(
    @Inject(DIALOG_DATA) public data: any, // Cambiado a any para aceptar isPurchase
    ...
    // ✅ Detectar si es compra
    if(data.isPurchase !== undefined){
