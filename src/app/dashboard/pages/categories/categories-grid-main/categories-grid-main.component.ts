...
@Component({
  ...
  imports: [HeaderComponent, SearchInputTextComponent, NgIcon, ChatBubbleComponent, NgClass,
     DialogModule,
     FormsModule],
  templateUrl: './categories-grid-main.component.html',
  styleUrl: './categories-grid-main.component.scss',
...
export default class CategoriesGridMainComponent extends BaseForm implements OnInit {
  ...
  edit(id: number){
    this.router.navigate([`/dashboard/categories/detail/edit/${id}`]);
  }

  add(){
    this.router.navigate([`/dashboard/categories/detail/new`]);
  }
  ...
  filter(name?: string, id?: number, initPrice?: number, endPrice?: number, active?: boolean){
    ...
    if(name){
      filter = filter.concat(`&categoryName=${name}`);
    }
